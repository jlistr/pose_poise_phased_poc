...
import { 
  ...
  RefreshCw, Cloud, CloudOff, Check, Box, Tally3, Plus, Minus, Target,
  Handshake, Plane, MapPin
} from 'lucide-react';
...
// Initial Static Data
const SEED_IMAGES = [
  { id: "1", url: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=1000&auto=format&fit=crop', title: 'Editorial Noir', category: 'Editorial', photoshoot: 'Shadow & Light', date: '2023-05-12', isStarred: false },
  { id: "2", url: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=1000&auto=format&fit=crop', title: 'Street Style', category: 'Urban', photoshoot: 'Downtown Motion', date: '2023-08-22', isStarred: true },
  { id: "3", url: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=1000&auto=format&fit=crop', title: 'Golden Hour', category: 'Fashion', photoshoot: 'Sunset Series', date: '2024-02-10', isStarred: false },
  { id: "4", url: 'https://images.unsplash.com/photo-1502685104226-ee32379fefbe?q=80&w=1000&auto=format&fit=crop', title: 'Vogue Concept', category: 'Editorial', photoshoot: 'Shadow & Light', date: '2023-05-12', isStarred: false },
  { id: "5", url: 'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?q=80&w=1000&auto=format&fit=crop', title: 'Minimalist Portrait', category: 'Beauty', photoshoot: 'Pure Form', date: '2024-01-05', isStarred: false },
  { id: "6", url: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=1000&auto=format&fit=crop', title: 'Summer Collection', category: 'Commercial', photoshoot: 'Coastal Breeze', date: '2023-07-15', isStarred: false },
  { id: "7", url: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1000&auto=format&fit=crop', title: 'Classic Suit', category: 'Classic', photoshoot: 'The Gentleman', date: '2023-11-30', isStarred: false },
  { id: "8", url: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=1000&auto=format&fit=crop', title: 'Neon Nights', category: 'Avant Garde', photoshoot: 'Cyberpunk Tokyo', date: '2024-04-12', isStarred: false },
  { id: "9", url: 'https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?q=80&w=1000&auto=format&fit=crop', title: 'High Fashion', category: 'Editorial', photoshoot: 'Sunset Series', date: '2024-02-10', isStarred: false },
  { id: "10", url: 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=1000&auto=format&fit=crop', title: 'Urban Jungle', category: 'Streetwear', photoshoot: 'Downtown Motion', date: '2023-08-22', isStarred: false },
  { id: "11", url: 'https://images.unsplash.com/photo-1529139513477-42f56679ac92?q=80&w=1000&auto=format&fit=crop', title: 'Silhouette', category: 'Fine Art', photoshoot: 'Pure Form', date: '2024-01-05', isStarred: false },
  { id: "12", url: 'https://images.unsplash.com/photo-1512436991641-6745cdb1723f?q=80&w=1000&auto=format&fit=crop', title: 'Monochrome', category: 'Commercial', photoshoot: 'The Gentleman', date: '2023-11-30', isStarred: false },
  { id: "13", url: '/hero_vid_neon_nights.mp4', title: 'Neon Action', category: 'Motion', photoshoot: 'Cyberpunk Tokyo', date: '2024-04-13', isStarred: false },
];

const SERVICES_DATA = [
  { id: '1', name: 'High-Fashion Editorial', rate: '$250/hr', description: 'Available for print & digital campaigns globally.', travel: true, tfp: false },
  { id: '2', name: 'Commercial & Brand Content', rate: '$150/hr', description: 'Social media, website, and promotional content.', travel: true, tfp: false },
  { id: '3', name: 'Runway & Showroom', rate: '$800/day', description: 'International runway and designer presentations.', travel: true, tfp: false },
  { id: '4', name: 'Creative Collaboration', rate: 'Contact', description: 'Selective TFP (Time for Print) for unique concepts.', travel: false, tfp: true },
];
...
const App = () => {
  ...
  // Data State
  const [galleryItems, setGalleryItems] = useState(SEED_IMAGES);
  ...
  const cycleItemSize = (id: string) => {
    triggerUserAction(() => {
        setGalleryItems(prev => prev.map(img => {
            if (img.id === id) {
                const current = (img as any).colSpan || 2;
    ...
    triggerUserAction(() => {
        setGalleryItems(prev => {
            ...
            return prev.map((img, i) => {
                if (i === idx) {
                    // Detect effective current span if not manually set
                    let current = (img as any)[dimension === 'col' ? 'colSpan' : 'rowSpan'];
                    if (!current) {
                      // Fallback to layout default if missing
                      const defaultSpan = currentLayout === LAYOUTS.BENTO ? getBentoSpan(i) : currentLayout === LAYOUTS.HIERARCHICAL ? getHierarchicalSpan(i) : 'col-span-1 row-span-1';
                      const match = defaultSpan.match(dimension === 'col' ? /col-span-(\d)/ : /row-span-(\d)/);
  ...
  const toggleItemStar = (id: string) => {
    triggerUserAction(() => {
        setGalleryItems(prev => prev.map(img => {
            if (img.id === id) {
                return { ...img, isStarred: !(img as any).isStarred };
            }
            return img;
        }));
    });
  ...
  const renderGallery = () => {
            ...
            <motion.section key={shoot.name} id={shoot.id} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} className="scroll-mt-32">
              <div className={`flex flex-col md:flex-row md:items-end justify-between border-b pb-6 ${isDarkMode ? 'border-neutral-800' : 'border-neutral-200'}`}>
                <div>
                  ...
                  <h3 className="text-5xl font-black uppercase tracking-tighter mt-2">{shoot.name}</h3>
                </div>
                <div className="flex items-center gap-8 mt-6 md:mt-0">
                    <span className={`text-4xl font-black italic opacity-40`} style={{ color: accentColor }}>/ 0{idx + 1}</span>
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12">
                {shoot.images.map((img: any) => (
                  <div key={img.id} className="aspect-[3/4]">
                    <PortfolioCard img={img} isDarkMode={isDarkMode} isFullHeight onClick={() => setSelectedImage(img)} onRemove={() => setImageToRemove(img)} onToggleStar={() => toggleItemStar(img.id)} />
                  </div>
                ))}
              </div>
            </motion.section>
    ...
    const layoutClasses = currentLayout === LAYOUTS.MASONRY ? "columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6" : 
                         currentLayout === LAYOUTS.HIERARCHICAL ? "grid grid-cols-1 md:grid-cols-4 auto-rows-[220px] gap-6 grid-flow-row-dense" : 
                         currentLayout === LAYOUTS.CLASSIC ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 auto-rows-[220px] gap-6 grid-flow-row-dense" :
                         "grid grid-cols-1 md:grid-cols-4 auto-rows-[220px] gap-6 grid-flow-row-dense";

    const getAspectSpan = (colSpan: number, rowSpan: number) => {
        return `col-span-${colSpan || 2} row-span-${rowSpan || 2}`;
    };
        ...
        <DragDropProvider onDragStart={() => setIsDragging(true)} onDragEnd={handleDragEnd}>
          <div className={layoutClasses}>
            {galleryItems.map((img, idx) => {
              return (
                <SortableItem key={img.id} id={img.id} index={idx} spanClass={((img as any).colSpan || (img as any).rowSpan) ? getAspectSpan((img as any).colSpan, (img as any).rowSpan) : currentLayout === LAYOUTS.BENTO ? ( (img as any).isStarred ? 'col-span-2 row-span-2' : getBentoSpan(idx) ) : currentLayout === LAYOUTS.HIERARCHICAL ? ( (img as any).isStarred ? 'md:col-span-3 md:row-span-2' : getHierarchicalSpan(idx) ) : ""}>
                  {(handleRef: any) => (
                    <PortfolioCard img={img} isDarkMode={isDarkMode} isFullHeight={currentLayout !== LAYOUTS.MASONRY} onClick={() => setSelectedImage(img)} isDraggable handleRef={handleRef} onRemove={() => setImageToRemove(img)} onCycleSize={() => cycleItemSize(img.id)} onUpdateSize={(dim: 'col' | 'row', delta: number) => updateItemSize(img.id, dim, delta)} onToggleStar={() => toggleItemStar(img.id)} />
                  )}
                </SortableItem>
              );
            })}
    ...
    <div className={`min-h-screen ${isDarkMode ? "bg-neutral-950 text-neutral-100" : "bg-neutral-50 text-neutral-900"} transition-colors duration-500 font-sans selection:bg-neutral-500 overflow-x-hidden`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className={`flex flex-col gap-3 p-4 rounded-[1.5rem] border ${borderClasses} ${isDarkMode ? 'bg-white/5' : 'bg-black/5'}`}>
                          ...
                          <div className="flex items-center gap-3">
                              ...
                              {(window as any).EyeDropper && <button onClick={() => openEyeDropper(setH1Color)} className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-black/10'}`}><Pipette className="w-3 h-3 opacity-60" /></button>}
                          </div>
                      </div>
                      ...
                      <div className={`flex flex-col gap-3 p-4 rounded-[1.5rem] border ${borderClasses} ${isDarkMode ? 'bg-white/5' : 'bg-black/5'}`}>
                          ...
                          <div className="flex items-center gap-3">
                              ...
                              {(window as any).EyeDropper && <button onClick={() => openEyeDropper(setAccentColor)} className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-black/10'}`}><Pipette className="w-3 h-3 opacity-60" /></button>}
                          </div>
                      </div>
                    </div>
                  </div>
      ...
      <nav className={`fixed top-0 w-full z-[80] transition-all duration-500 border-b ${glassClasses}`}>
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex flex-1 items-center justify-center gap-10 md:gap-14">{[{ name: 'Bio', id: 'bio' }, { name: 'Portfolio', id: 'gallery-anchor' }, { name: 'The Terms', id: 'terms' }, { name: 'Services', id: 'services' }, { name: 'Inquire', id: 'inquire' }].map(tab => (<button key={tab.name} onClick={() => scrollTo(tab.id, 100)} className="text-[11px] uppercase tracking-[0.4em] font-black opacity-40 hover:opacity-100 transition-opacity whitespace-nowrap">{tab.name}</button>))}</div>
      ...
      <main className="px-8 pb-32 max-w-7xl mx-auto relative">
        ...
        <div className={`flex items-center justify-between mb-12 pb-8 border-b`} style={{ borderColor: `${accentColor}30` }}><h2 className={`text-xs uppercase tracking-[0.8em] font-black ${isDarkMode ? 'text-neutral-600' : 'text-neutral-300'}`}>{viewMode === VIEW_MODES.GALLERY ? 'Signature Portfolio' : 'The Anthology'}</h2><div className={`flex gap-8 text-[11px] uppercase tracking-widest font-black`} style={{ color: accentColor }}><span>{galleryItems.length} Frames</span><span>/</span><span>Archives 23:25</span></div></div>
        ...
        <section id="terms" className="mt-72 pt-32 border-t border-neutral-800/10">
            <h3 className="text-5xl font-black uppercase tracking-tighter mb-12 flex items-center gap-6">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-shield-check w-12 h-12 opacity-20" aria-hidden="true">
                    <path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"></path>
                    <path d="m9 12 2 2 4-4"></path>
                </svg> 
                The Terms
            </h3>
            <p className="text-lg leading-relaxed max-w-2xl text-neutral-500">Usage rights strictly editorial unless negotiated otherwise. Bookings require 14 days lead.</p>
        </section>
        
        {/* SERVICES & RATES SECTION (REFACTORED - ONBOARDING STYLE) */}
        <section id="services" className="mt-48 relative">
            <div className="flex flex-col mb-16">
                <span className={`text-[11px] uppercase tracking-[0.8em] font-black ${isDarkMode ? 'text-neutral-600' : 'text-neutral-400'}`}>02 // Professional Portfolio</span>
                <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter mt-4 leading-none text-white">Your Services</h2>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-24">
                <div className="space-y-16">
                    {/* Experience Level */}
                    <div>
                        <h4 className="text-[10px] uppercase tracking-[0.4em] font-black opacity-40 mb-8">Experience Level</h4>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { id: 'beginner', title: 'Beginner', desc: 'New to modeling, building portfolio', checked: false },
                                { id: 'intermediate', title: 'Intermediate', desc: '1-3 years experience', checked: false },
                                { id: 'professional', title: 'Professional', desc: '3+ years, established career', checked: true },
                                { id: 'expert', title: 'Expert', desc: 'Top-tier, international work', checked: false }
                            ].map(level => (
                                <div key={level.id} className={`p-6 rounded-3xl border transition-all duration-500 ${level.checked ? (isDarkMode ? 'bg-amber-500/10 border-amber-500/50' : 'bg-amber-50/50 border-amber-500/50') : (borderClasses + ' ' + (isDarkMode ? 'bg-white/5' : 'bg-black/5'))} relative`}>
                                    <h5 className="text-sm font-black uppercase tracking-wide mb-1">{level.title}</h5>
                                    <p className="text-[10px] opacity-40 font-medium leading-relaxed">{level.desc}</p>
                                    {level.checked && <div className="absolute top-4 right-4 w-5 h-5 bg-amber-500 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(245,158,11,0.5)]"><Check className="w-3 h-3 text-white" /></div>}
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Service Categories */}
                    <div>
                        <h4 className="text-[10px] uppercase tracking-[0.4em] font-black opacity-40 mb-8">Service Categories</h4>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { id: 'fashion', title: 'Fashion', desc: 'Runway, editorial, commercial fashion', checked: true },
                                { id: 'commercial', title: 'Commercial', desc: 'Advertising, product, lifestyle', checked: true },
                                { id: 'editorial', title: 'Editorial', desc: 'Magazine, artistic, conceptual', checked: true },
                                { id: 'fitness', title: 'Fitness', desc: 'Athletic wear, sports, wellness', checked: true },
                                { id: 'glamour', title: 'Glamour', desc: 'Beauty, swimwear, lingerie', checked: true },
                                { id: 'portrait', title: 'Portrait', desc: 'Headshots, personal branding', checked: true }
                            ].map(cat => (
                                <div key={cat.id} className={`p-6 rounded-3xl border transition-all duration-500 ${cat.checked ? (isDarkMode ? 'bg-amber-500/10 border-amber-500/50' : 'bg-amber-50/50 border-amber-500/50') : (borderClasses + ' ' + (isDarkMode ? 'bg-white/5' : 'bg-black/5'))} relative`}>
                                    <h5 className="text-sm font-black uppercase tracking-wide mb-1">{cat.title}</h5>
                                    <p className="text-[10px] opacity-40 font-medium leading-relaxed">{cat.desc}</p>
                                    {cat.checked && <div className="absolute top-4 right-4 w-5 h-5 bg-amber-500 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(245,158,11,0.5)]"><Check className="w-3 h-3 text-white" /></div>}
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="space-y-16">
                    {/* Rates */}
                    <div>
                        <h4 className="text-[10px] uppercase tracking-[0.4em] font-black opacity-40 mb-8">Base Rates (Starting At)</h4>
                        <div className="space-y-4">
                            {[
                                { label: 'Hourly', value: '150' },
                                { label: 'Half Day (4hr)', value: '500' },
                                { label: 'Full Day (8hr)', value: '900' }
                            ].map(rate => (
                                <div key={rate.label} className={`p-6 rounded-3xl border ${borderClasses} ${isDarkMode ? 'bg-white/5' : 'bg-black/5'} flex items-center justify-between`}>
                                    <span className="text-xs font-black uppercase tracking-widest opacity-60">{rate.label}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black opacity-30">$</span>
                                        <span className="text-2xl font-black italic" style={{ color: accentColor }}>{rate.value}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className={`p-8 rounded-[2.5rem] border ${borderClasses} ${isDarkMode ? 'bg-white/5' : 'bg-black/5'} flex items-center justify-between`}>
                            <div className="flex items-center gap-6">
                                <div className="w-12 h-12 rounded-2xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20"><Plane className={`w-5 h-5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} /></div>
                                <div>
                                    <h5 className="text-[11px] font-black uppercase tracking-widest text-white leading-none">Available for Travel</h5>
                                    <p className="text-[9px] opacity-40 uppercase tracking-tight mt-1.5 font-bold">Willing to travel for bookings</p>
                                </div>
                            </div>
                            <div className="w-14 h-7 rounded-full bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)] relative flex items-center justify-end px-1"><div className="w-5 h-5 bg-white rounded-full shadow-md" /></div>
                        </div>
                        <div className={`p-8 rounded-[2.5rem] border ${borderClasses} ${isDarkMode ? 'bg-white/5' : 'bg-black/5'} flex items-center justify-between`}>
                            <div className="flex items-center gap-6">
                                <div className="w-12 h-12 rounded-2xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20"><Handshake className={`w-5 h-5 text-amber-500`} /></div>
                                <div>
                                    <h5 className="text-[11px] font-black uppercase tracking-widest text-white leading-none">Open to TFP/Collab</h5>
                                    <p className="text-[9px] opacity-40 uppercase tracking-tight mt-1.5 font-bold">Trade for prints / collaboration shoots</p>
                                </div>
                            </div>
                            <div className="w-14 h-7 rounded-full bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)] relative flex items-center justify-end px-1"><div className="w-5 h-5 bg-white rounded-full shadow-md" /></div>
                        </div>
            ...
            <div className="mt-32 flex justify-center">
                <button onClick={() => scrollTo('inquire')} className={`px-16 py-6 rounded-full text-[11px] font-black uppercase tracking-[0.4em] transition-all flex items-center gap-4 shadow-2xl hover:scale-105 active:scale-95`} style={{ backgroundColor: accentColor, color: '#ffffff' }}>Contact for Professional Booking</button>
            </div>
        </section>
          ...
          <div className={`p-10 md:p-20 rounded-[4rem] text-left border shadow-2xl transition-all duration-700 ${glassClasses}`} style={{ borderColor: `${accentColor}30` }}>
            <div className="flex flex-col lg:flex-row gap-16">
              <div className="flex-1">
                ...
                <h3 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-6 italic" style={{ color: h1Color }}>Inquire.</h3>
      ...
      <AnimatePresence>{selectedImage && <ImageModal image={selectedImage} isDarkMode={isDarkMode} onClose={() => setSelectedImage(null)} />}</AnimatePresence>
      <AnimatePresence>{imageToRemove && <ConfirmRemoveModal image={imageToRemove} onConfirm={confirmRemoval} onCancel={() => setImageToRemove(null)} isDarkMode={isDarkMode} />}</AnimatePresence>
    </div>
  );
};
...
const EdgeResizeHandle = ({ side, onResize }: { side: 'left' | 'right', onResize: (delta: number) => void }) => {
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        const handlePointerMove = (moveEvent: PointerEvent) => {
            ...
            if (side === 'left') gridDelta = -gridDelta;
    ...
    return (
        <div 
            ...
            className={`absolute top-20 bottom-20 ${side}-0 w-10 z-40 opacity-0 group-hover:opacity-100 hover:bg-white/30 transition-all pointer-events-auto flex items-center justify-center cursor-ew-resize`}
        >
            <div className="w-1.5 h-12 rounded-full bg-white/60 shadow-[0_0_10px_rgba(255,255,255,0.3)]" />
        </div>
    );
};

const PortfolioCard = ({ img, isDarkMode, onClick, isDraggable, onRemove, onCycleSize, onUpdateSize, onToggleStar, handleRef }: any) => (
  <div className={`group relative overflow-hidden rounded-[3rem] w-full h-full transition-all duration-700 ${img.isStarred ? 'ring-4 ring-amber-500/50 shadow-[0_0_50px_rgba(245,158,11,0.2)]' : ''} ${isDarkMode ? 'bg-neutral-900 border-white/10' : 'bg-white border-black/5'} border shadow-sm`}>
    {isDraggable && onUpdateSize && (
      <>
        <EdgeResizeHandle side="left" onResize={(d: number) => onUpdateSize('col', d)} />
        <EdgeResizeHandle side="right" onResize={(d: number) => onUpdateSize('col', d)} />
      </>
    )}
    <div onClick={onClick} className="w-full h-full cursor-zoom-in relative">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/95 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-12 pointer-events-none">
            <span className="text-[10px] text-white/40 uppercase tracking-[0.4em] font-black mb-2">{img.category}</span>
            <h4 className="text-2xl font-black uppercase tracking-widest text-white leading-tight">{img.title}</h4>
        </div>
    </div>
    <div className="absolute top-8 left-8 right-8 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-3 group-hover:translate-y-0 z-20 pointer-events-none">
        <div className="flex gap-2 pointer-events-auto">
            ...
            <button 
              ...
              className={`backdrop-blur-2xl p-4 rounded-2xl border border-white/20 shadow-2xl transition-all ${img.isStarred ? 'bg-amber-500 border-amber-300 ring-2 ring-amber-500/20' : 'bg-black/80 hover:bg-black/95'}`}
              title={img.isStarred ? "Remove Focus" : "Apply Focus"}
            >
              <Star className={`w-4 h-4 transition-all ${img.isStarred ? 'fill-white text-white scale-110' : 'text-white/90'}`} />
            </button>
        </div>
        <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="bg-red-500/90 hover:bg-red-600 backdrop-blur-2xl p-4 rounded-2xl border border-white/10 shadow-2xl transition-all scale-90 hover:scale-100 pointer-events-auto"><Trash2 className="w-4 h-4 text-white" /></button>
    </div>
    {img.isStarred && (
        <div className="absolute top-6 right-6 z-10">
            <div className="bg-amber-500 p-2 rounded-full shadow-[0_0_15px_rgba(245,158,11,0.5)] animate-pulse">
...
const ImageModal = ({ image, onClose, isDarkMode }: any) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-md flex items-center justify-center p-8" onClick={onClose}>
    <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative max-w-6xl w-full flex flex-col items-center gap-12" onClick={e => e.stopPropagation()}>
      ...
      <div className="text-center"><span className="text-white/20 text-[11px] uppercase tracking-[0.8em] mb-4 block font-black">{image.photoshoot}</span><h3 className="text-6xl font-black uppercase tracking-tighter text-white drop-shadow-2xl">{image.title}</h3></div>
      <button onClick={onClose} className="mt-4 px-16 py-6 bg-white text-black rounded-full text-[11px] font-black uppercase tracking-[0.4em] shadow-2xl hover:scale-105 active:scale-95 transition-all">Dismiss View</button>
    </motion.div>
  </motion.div>
);

const ConfirmRemoveModal = ({ image, onConfirm, onCancel, isDarkMode }: any) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-8">
    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className={`max-w-lg w-full p-12 rounded-[4rem] border shadow-[0_40px_100px_rgba(0,0,0,0.8)] ${isDarkMode ? 'bg-black/90 border-white/10 text-white' : 'bg-white border-black/5 text-black'}`}>
        <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 italic">Curation Decision.</h3>
