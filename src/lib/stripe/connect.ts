...
export async function getAccountStatus(
  ...
  const entries = account.requirements?.entries as
    | Array<{
        state?: string;
  ...
  const pendingRequirements =
    entries
      ?.filter(
        (entry) => entry.state === "required" || entry.state === "optional",
      )
      ?.map(
        (entry) =>
          entry.acquirer_reference_number ||
          entry.type ||
          "Unknown requirement",
      ) ?? [];
  ...
  return {
    ...
    requirementsStatus: (requirementsStatus ||
      "pending") as AccountStatus["requirementsStatus"],
    pendingRequirements:
      pendingRequirements.length > 0 ? pendingRequirements : undefined,
  };
}
 ...
 */
export const PLATFORM_SUBSCRIPTION_PRICE_ID =
 ...
 */
export const DEFAULT_APPLICATION_FEE_PERCENT = 0.1;
