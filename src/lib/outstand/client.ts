...
class OutstandClient {
  ...
  async getPendingConnection(
    state: string,
  ): Promise<OutstandPendingConnection> {
  ...
  async getAccount(accountId: string): Promise<OutstandSocialAccount> {
    return this.request<OutstandSocialAccount>(`/social-accounts/${accountId}`);
  }
  ...
  async disconnectAccount(accountId: string): Promise<{ success: boolean }> {
    return this.request<{ success: boolean }>(`/social-accounts/${accountId}`, {
      method: "DELETE",
    });
  }
