...
import { colors, TEMPLATES, fonts, type PortfolioPhoto } from "../types";
...
function SortablePhotoCard({
  ...
  return (
    <div
      ...
      style={{
        ...style,
        border: `1px solid ${hasError ? colors.error : isDragging ? "var(--color-brand-gold)" : colors.border}`,
        borderRadius: 0,
        overflow: "hidden",
        ...
        >
          ...
          <div style={{ marginBottom: "0.5rem" }}>
            ...
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                onUpdateCredit(photo.id, "photographer", e.target.value)
              }
              disabled={isUploading}
              style={{
                width: "100%",
          ...
          <div>
            ...
            <input
              ...
              value={photo.studio}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                onUpdateCredit(photo.id, "studio", e.target.value)
              }
              disabled={isUploading}
              style={{
                width: "100%",
...
export function PhotosStep({
  ...
  const template =
    TEMPLATES.find((t) => t.id === selectedTemplate) || TEMPLATES[1];
  ...
  const handleAIDetectionFileChange = async (
    ...
    if (validFiles.length === 0) {
      alert(
        "Please upload valid image files (JPEG, PNG, WebP, GIF) under 10MB.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        result.extractedData?.photos &&
        result.extractedData.photos.length > 0
      ) {
        setDetectionResult({
          success: true,
          message:
            result.message ||
            `Found potential credits in ${result.extractedData.photos.filter((p: PhotoCredit) => p.photographer || p.studio).length} of ${base64Images.length} photos.`,
        ...
        setDetectionResult({
          ...
          message:
            result.message ||
            "I couldn't detect any photographer watermarks or studio logos in these photos. You can manually add credits to each photo below.",
          credits: [],
        });
      }
    } catch (error) {
      ...
      setDetectionResult({
        success: false,
        message:
          "I had trouble analyzing these photos. Please try again or add credits manually.",
        credits: [],
      });
    } finally {
  ...
  return (
    <>
      ...
      >
        Upload your best work. High-quality images make a strong first
        impression.
      </p>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        {!showAIPanel ? (
          ...
          >
            <div style={{ color: "var(--color-brand-gold)" }}>
            ...
            <div style={{ textAlign: "left", flex: 1 }}>
              ...
              >
                AI scans your photos for watermarks, signatures, and studio
                logos to auto-credit photographers
              </p>
            </div>
            <div
              style={{
          ...
          >
            ...
            {isDetecting && (
              ...
              >
                ...
                >
                  Scanning {detectionPhotos.length} photo(s) for watermarks and
                  signatures...
                </span>
              </div>
            )}
            ...
            {detectionResult && !isDetecting && (
              <div>
                ...
                {detectionResult.success &&
                  detectionResult.credits.length > 0 && (
                    <div
                      style={{
                        display: "grid",
                        gap: "0.75rem",
        ...
        >
          ...
          >
            AI looks for watermarks, signatures, and logos in your photos.
            Detection works best with clear, high-resolution images. You can
            always manually add or edit credits on each photo.
          </p>
        </div>
      </div>
      ...
      {/* Upload Area */}
      <div
        onClick={photos.length >= 10 ? undefined : handleUploadClick}
        onKeyDown={(e) => photos.length < 10 && e.key === "Enter" && handleUploadClick()}
        role={photos.length >= 10 ? "presentation" : "button"}
        tabIndex={photos.length >= 10 ? -1 : 0}
        style={{
          border: `2px dashed ${photos.length >= 10 ? colors.border : colors.border}`,
          borderRadius: 0,
          padding: "3rem 2rem",
          textAlign: "center",
          cursor: photos.length >= 10 ? "not-allowed" : "pointer",
          backgroundColor: photos.length >= 10 ? colors.border : "var(--color-brand-light)",
          transition: "border-color 0.2s",
          marginBottom: "2rem",
          opacity: photos.length >= 10 ? 0.6 : 1,
        }}
        onMouseEnter={(e) => {
          if (photos.length < 10) e.currentTarget.style.borderColor = "var(--color-brand-gold)";
        }}
        onMouseLeave={(e) => {
          if (photos.length < 10) e.currentTarget.style.borderColor = colors.border;
        }}
      >
        <input
          ref={fileInputRef}
        ...
        >
          {photos.length >= 10 ? "Maximum limit reached (10/10)" : "Click to upload or drag and drop"}
        </p>
        <p
          style={{
            fontSize: "12px",
        ...
        >
          {photos.length >= 10 ? "Upgrade to PRO to add more photos." : `JPEG, PNG, or WebP up to 10MB each (${photos.length}/10 Uploaded)`}
        </p>
      </div>
      ...
      {photos.length > 0 && isUploading && (
        ...
        >
          ...
          <span style={{ fontSize: "13px", color: colors.textSecondary }}>
            Uploading photos... {uploadProgress.uploaded} of{" "}
