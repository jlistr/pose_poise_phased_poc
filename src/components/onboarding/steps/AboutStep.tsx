...
import { colors, fonts, type ModelStats, type AboutData } from "../types";
...
export function AboutStep({
  ...
  const [compCardResult, setCompCardResult] = useState<AIAnalysisResult | null>(
    null,
  );
  ...
  const handleRemoveProfilePhoto = () => {
    if (data.profilePhoto?.startsWith("blob:")) {
  ...
  const handlePhotoAnalysisFilesSelect = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      if (
        ...
        if (result.extractedData.hairColor)
          extractedStats.hairColor = result.extractedData.hairColor;
        if (result.extractedData.eyeColor)
          extractedStats.eyeColor = result.extractedData.eyeColor;
        if (result.extractedData.heightCm)
          extractedStats.height = `${Math.round(Number(result.extractedData.heightCm) / 2.54)}"`;

        setPhotoAnalysisResult({
          success: true,
          message:
            result.message ||
            `Analysis complete! I analyzed ${base64Images.length} photo(s) and detected some characteristics.`,
        ...
        setPhotoAnalysisResult({
          ...
          message:
            result.message ||
            "I couldn't extract reliable measurements from these photos. For best results, please include a full-body shot with a reference object (like a door frame) and a close-up for hair/eye color, or enter your measurements manually below.",
        });
      }
    } catch (error) {
      ...
      setPhotoAnalysisResult({
        success: false,
        message:
          "I had trouble analyzing these photos. Please try different images or enter your measurements manually.",
      });
    } finally {
  ...
  const handleApplyPhotoAnalysis = () => {
    if (photoAnalysisResult?.extractedData) {
      ...
      setPhotoAnalysisResult((prev) =>
        prev ? { ...prev, message: "✓ Applied to your profile!" } : null,
      );
    }
  };
  ...
  const handlePhotoAnalysisInputChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  const handleCompCardFileSelect = useCallback(async (file: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/onboarding/ai-chat", {
        ...
        body: JSON.stringify({
          message:
            "Please scan this comp card and extract all modeling statistics.",
          step: "about",
          tool: "comp-scanner",
          images: [base64],
        }),
      ...
      if (
        ...
        const extractedStats: Partial<ModelStats> = {};
        if (result.extractedData.heightCm)
          extractedStats.height = `${Math.round(Number(result.extractedData.heightCm) / 2.54 / 12)}'${Math.round(Number(result.extractedData.heightCm) / 2.54) % 12}"`;
        if (result.extractedData.bustCm)
          extractedStats.bust = `${Math.round(Number(result.extractedData.bustCm) / 2.54)}"`;
        if (result.extractedData.waistCm)
          extractedStats.waist = `${Math.round(Number(result.extractedData.waistCm) / 2.54)}"`;
        if (result.extractedData.hipsCm)
          extractedStats.hips = `${Math.round(Number(result.extractedData.hipsCm) / 2.54)}"`;
        if (result.extractedData.shoeSize)
          extractedStats.shoes = result.extractedData.shoeSize;
        if (result.extractedData.hairColor)
          extractedStats.hairColor = result.extractedData.hairColor;
        if (result.extractedData.eyeColor)
          extractedStats.eyeColor = result.extractedData.eyeColor;
        ...
        setCompCardResult({
          ...
          message:
            result.message ||
            "Successfully extracted your stats from the comp card!",
          extractedData: extractedStats,
          confidence:
            result.confidence > 0.7
              ? "high"
        ...
        setCompCardResult({
          ...
          message:
            result.message ||
            "I couldn't read the stats from this comp card. Please make sure the text is clear and visible, or enter your measurements manually below.",
        });
      }
    } catch (error) {
      ...
      setCompCardResult({
        success: false,
        message:
          "I had trouble scanning this comp card. Please try a clearer image or enter your measurements manually.",
      });
    } finally {
  ...
  const handleCompCardInputChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (file) {
  ...
  const clearCompCard = () => {
    if (compCardFile?.startsWith("blob:")) {
  ...
  const dropZoneStyle = (
    ...
    border: isActive
      ? `2px dashed var(--color-brand-gold)`
  ...
  return (
    <>
      ...
      >
        Share your measurements and a bit about yourself. This helps clients
        find the right fit.
      </p>
        ...
        >
          ...
          <div
            style={{
              ...
              height: "80px",
              borderRadius: "50%",
              backgroundColor: data.profilePhoto
                ? "transparent"
                : colors.border,
              display: "flex",
              alignItems: "center",
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              {isProfileDragActive
                ...
                : "Upload a professional headshot for your About page"}
            </p>
            <button
              type="button"
              onClick={(e) => {
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        >
          Save time by letting AI extract your measurements. Choose one of the
          options below:
        </p>
        ...
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
          ...
          <div style={aiToolCardStyle}>
            <div
              ...
              onClick={
                analysisPhotos.length === 0 && !isAnalyzingPhoto
                  ? handlePhotoAnalysisUpload
                  : undefined
              }
              style={{
                ...
                >
                  ...
                  {isAnalyzingPhoto ? (
                    ...
                    >
                      ...
                      >
                        Analyzing {analysisPhotos.length} photo(s)...
                      </span>
                    </div>
                  ) : (
                    photoAnalysisResult && (
                      <div style={{ fontSize: "13px", fontFamily: fonts.body }}>
                        <p
                          style={{
                            color: photoAnalysisResult.success
                              ? colors.textPrimary
                        ...
                        {photoAnalysisResult.success &&
                          photoAnalysisResult.extractedData && (
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleApplyPhotoAnalysis();
                              }}
                              style={{
                                padding: "0.5rem 1rem",
                                fontSize: "12px",
              ...
              >
                ...
                >
                  ...
                  <ul style={{ margin: "0.25rem 0 0", paddingLeft: "1rem" }}>
                    <li>
                      <strong>Full-body shot</strong> standing straight next to
                      a door or furniture
                    </li>
                    <li>
                      <strong>Close-up</strong> of your face for hair and eye
                      color
                    </li>
          ...
          <div style={aiToolCardStyle}>
            <div
              ...
              onClick={
                !compCardFile && !isScanningCompCard
                  ? handleCompCardUpload
                  : undefined
              }
              style={{
            ...
            >
              ...
              {compCardFile ? (
                  ...
                  {isScanningCompCard ? (
                    ...
                    compCardResult && (
                      <div style={{ fontSize: "13px", fontFamily: fonts.body }}>
                        ...
                        {compCardResult.success &&
                          compCardResult.extractedData && (
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleApplyCompCardData();
                              }}
                              style={{
                                padding: "0.5rem 1rem",
                                fontSize: "12px",
                ...
                >
                  ...
                  >
                    {isCompCardDragActive
                      ? "Drop your comp card here..."
                      : "Drag & drop or click to upload"}
                  </p>
                  <p
                    style={{
              ...
              >
                ...
                >
                  ...
                  <p style={{ margin: "0.25rem 0 0" }}>
                    Upload your existing comp card and I&apos;ll extract all
                    your stats automatically.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        >
          Enter your measurements manually, or use the AI tools above to
          auto-fill.
        </p>
        <div
          style={{
