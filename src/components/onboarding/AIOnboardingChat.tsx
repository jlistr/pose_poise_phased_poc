...
const Icons = {
  ...
  photoAnalysis: (
    ...
    >
      ...
      {/* AI Sparkles */}
      <path
        d="M38 8l1 3 3 1-3 1-1 3-1-3-3-1 3-1 1-3"
        strokeWidth="1.5"
        fill="currentColor"
      />
      <path
        d="M8 6l0.7 2 2 0.7-2 0.7L8 12l-0.7-2.6-2-0.7 2-0.7L8 6"
        strokeWidth="1"
        fill="currentColor"
      />
    </svg>
  ...
  bioGenerator: (
    ...
    >
      ...
      {/* AI Sparkle */}
      <path
        d="M40 4l1.5 4 4 1.5-4 1.5-1.5 4-1.5-4-4-1.5 4-1.5 1.5-4"
        strokeWidth="1.5"
        fill="currentColor"
      />
    </svg>
  ...
  servicesSuggest: (
    ...
    >
      ...
      {/* AI Sparkle */}
      <path
        d="M8 8l1 3 3 1-3 1-1 3-1-3-3-1 3-1 1-3"
        strokeWidth="1"
        fill="currentColor"
      />
    </svg>
...
const STEP_TOOLS: Record<OnboardingStep, Tool[]> = {
  profile: [
    ...
    },
    {
      ...
      name: "Agency Details",
      description:
        "Add your representation details for professional credibility",
      icon: Icons.agency,
      action: "click",
    },
  ],
  about: [
    {
      ...
      description:
        "Upload a photo and let AI extract your measurements, hair color, and eye color automatically",
    ...
    },
    {
      ...
      description:
        "Scan your existing comp card to import all your stats instantly",
    ...
    },
    {
      ...
      name: "AI Bio Writer",
      description:
        "Generate a compelling professional bio based on your details",
  ...
  services: [
    {
      ...
      description:
        "Get personalized service recommendations with competitive pricing for your market",
    ...
    },
    {
      ...
      description:
        "Create a professional comp card using your profile and best photos",
...
const styles = {
  ...
  toolCard: (highlight: boolean = false, active: boolean = false) => ({
    ...
    border: active
      ? "2px solid #C4A484"
      : highlight
        ? "1px solid rgba(196, 164, 132, 0.3)"
...
const STEP_CONFIG: Record<OnboardingStep, { title: string; subtitle: string }> =
  {
    profile: {
      title: "Your Profile",
      subtitle:
        "Let's establish your professional identity. Use our tools to quickly set up your profile.",
    },
    about: {
      title: "About You",
      subtitle:
        "Share your story and stats. Our AI tools can extract your measurements from photos or comp cards.",
    },
    services: {
      title: "Your Services",
      subtitle:
    ...
    template: {
      ...
      subtitle:
        "Select a stunning template that reflects your unique style and brand.",
    },
    photos: {
...
export function AIOnboardingChat({
  ...
  const [_isAnalyzingImage, setIsAnalyzingImage] = useState(false);
  ...
  const [_toolCompleted, setToolCompleted] = useState<Record<string, boolean>>(
    {},
  );
  ...
  useEffect(() => {
    ...
    extractedData.tiktok,
    extractedData.website,
    extractedData.agencyName,
    extractedData.agencyWebsite,
    extractedData.agencyContact,
  ]);
  ...
  const progressToNextTool = useCallback(() => {
    ...
    if (nextIndex < currentToolSequence.length) {
      ...
      const nextTool = STEP_TOOLS[currentStep]?.find(
        (t) => t.id === nextToolId,
      );
  ...
  const completeCurrentTool = useCallback(
    (toolId: string) => {
  ...
  const skipCurrentTool = useCallback(() => {
    ...
    if (currentToolId) {
      addAssistantMessage(
        "No problem, we can skip this for now. You can always update it later.",
      );
  ...
  const handleToolClick = async (tool: Tool) => {
    ...
    if (tool.action === "upload") {
      ...
      if (tool.id === "photo-analyzer") {
        addAssistantMessage(
          "Please upload a full-body photo and I'll analyze it to extract your measurements, hair color, and eye color. For best results, use a well-lit photo where you're standing straight.",
        );
      } else if (tool.id === "comp-scanner") {
        addAssistantMessage(
          "Upload your existing comp card (image or PDF) and I'll extract all your stats automatically - including measurements, hair color, eye color, and any agency information.",
        );
      } else if (tool.id === "photo-organizer") {
        addAssistantMessage(
          "Upload your portfolio photos and I'll scan them for photographer watermarks, signatures, and studio logos. This helps you auto-credit the creative team for each image.",
        );
      }
      ...
      if (tool.id === "bio-generator") {
        ...
        handleAIRequest(
          "services-suggest",
          "Suggest modeling services with pricing for my market",
        );
  ...
  const handleGetLocation = async () => {
    ...
    if (!navigator.geolocation) {
      addAssistantMessage(
        "Geolocation is not supported by your browser. Please type your city and country, or click 'Skip' to continue.",
      );
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          setIsTyping(false);
          addAssistantMessage(
            "I couldn't determine your exact location. Please type your city and country, or click 'Skip' to continue.",
          );
          locationRequestInProgressRef.current = false;
      ...
      () => {
        setIsTyping(false);
        addAssistantMessage(
          "Location access was denied. Please type your city and country (e.g., 'Los Angeles, USA'), or click 'Skip' to continue.",
        );
        locationRequestInProgressRef.current = false;
  ...
  const handleAIRequest = async (tool: string, message: string) => {
    ...
    try {
      ...
      addAssistantMessage(
        "I encountered an issue. Please try again or provide the information manually.",
      );
    } finally {
      setIsTyping(false);
      setActiveTool(null);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!userMessageText && pendingImages.length > 0) {
      if (activeTool === "photo-analyzer") {
        ...
        userMessageText =
          "Please scan this comp card and extract all my information";
      } else {
    ...
    try {
      ...
      if (data.extractedData && Object.keys(data.extractedData).length > 0) {
        ...
        if (
          ...
          (activeTool === "photo-analyzer" || activeTool === "comp-scanner")
        ) {
      ...
      addAssistantMessage(
        "I apologize, but I encountered an issue processing your request. You can try again or enter your information manually.",
      );
    } finally {
      setIsTyping(false);
  ...
  const handleSaveSocials = () => {
    ...
    if (socialHandles.instagram)
      updates.instagram = socialHandles.instagram.replace("@", "");
    if (socialHandles.tiktok)
      updates.tiktok = socialHandles.tiktok.replace("@", "");
  ...
  const handleSaveAgency = () => {
    if (agencyDetails.name) {
      ...
      addAssistantMessage(
        `✓ Got it! I've noted that you're currently not represented by an agency.`,
      );
    }
  ...
  const extractedDataItems = [
    extractedData.displayName && {
      label: "Name",
      value: extractedData.displayName,
    },
    ...
    extractedData.heightCm && {
      label: "Height",
      value: `${extractedData.heightCm} cm`,
    },
    extractedData.bustCm && {
      label: "Bust",
      value: `${extractedData.bustCm} cm`,
    },
    extractedData.waistCm && {
      label: "Waist",
      value: `${extractedData.waistCm} cm`,
    },
    extractedData.hipsCm && {
      label: "Hips",
      value: `${extractedData.hipsCm} cm`,
    },
    extractedData.shoeSize && { label: "Shoe", value: extractedData.shoeSize },
    extractedData.hairColor && {
      label: "Hair",
      value: extractedData.hairColor,
    },
    extractedData.eyeColor && { label: "Eyes", value: extractedData.eyeColor },
    extractedData.bio && {
      label: "Bio",
      value:
        extractedData.bio.length > 60
          ? `${extractedData.bio.substring(0, 60)}...`
          : extractedData.bio,
    },
  ...
  return (
    <div style={styles.container}>
      ...
      <input
        ...
        type="file"
        accept={
          activeTool === "comp-scanner" ? "image/*,application/pdf" : "image/*"
        }
      ...
      <div style={styles.toolsPanel}>
        ...
        <div style={styles.toolsList}>
          {currentTools.map((tool) => (
            ...
            >
              <div style={styles.toolIcon(tool.highlight)}>{tool.icon}</div>
              <div style={styles.toolInfo}>
                <div style={styles.toolName}>
                  {tool.name}
                  {tool.highlight && <span style={styles.aiBadge}>AI</span>}
                </div>
          ...
          {currentStep === "services" && subscriptionTier === "free" && (
            <div
              style={{
                ...styles.toolCard(false, false),
                opacity: 0.5,
                cursor: "not-allowed",
              }}
            >
              <div style={styles.toolIcon(false)}>{Icons.compGenerator}</div>
              <div style={styles.toolInfo}>
                <div style={styles.toolName}>
                  Comp Card Generator
                  <span
                    style={{ ...styles.aiBadge, backgroundColor: "#1A1A1A" }}
                  >
                    PRO
                  </span>
                </div>
            ...
            {hasExtractedData && (
              <div style={{ position: "relative" }}>
                <div
                  ...
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = showDataSummary
                      ? "#C4A484"
                      : "#1A1A1A";
                  }}
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          bottom: 0,
                          background:
                            "linear-gradient(90deg, transparent 0%, rgba(196, 164, 132, 0.08) 50%, transparent 100%)",
                      ...
                      >
                        <div
                          style={{
                            ...
                            borderRadius: "10px",
                            background:
                              "linear-gradient(135deg, rgba(196, 164, 132, 0.3) 0%, rgba(196, 164, 132, 0.15) 100%)",
                            display: "flex",
                            alignItems: "center",
                            justifyContent: "center",
                        ...
                        <div>
                          ...
                          >
                            ...
                            />
                            {extractedDataItems.length} field
                            {extractedDataItems.length !== 1 ? "s" : ""}{" "}
                            collected
                          </div>
                        </div>
                      </div>
                    ...
                    >
                      {extractedDataItems.map((item, i) => (
                        <div
                          ...
                          style={{
                            marginBottom:
                              i < extractedDataItems.length - 1 ? "14px" : 0,
                    ...
                    <div
                      style={{
                        ...
                        background:
                          "linear-gradient(180deg, rgba(196, 164, 132, 0.03) 0%, rgba(196, 164, 132, 0.08) 100%)",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "space-between",
                        position: "relative",
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          bottom: 0,
                          background:
                            "linear-gradient(90deg, transparent 0%, rgba(196, 164, 132, 0.1) 50%, transparent 100%)",
                          backgroundSize: "200% 100%",
                          pointerEvents: "none",
                        }}
                      />
        ...
        <div ref={chatContainerRef} style={styles.messagesContainer}>
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => {
                    ...
                    alert(
                      "Instagram connection coming soon! For now, enter your handle below.",
                    );
                  }}
                  style={{
                    flex: 1,
                ...
                <button
                  onClick={() => {
                    ...
                    alert(
                      "TikTok connection coming soon! For now, enter your handle below.",
                    );
                  }}
                  style={{
                    flex: 1,
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                {socialHandles.website && socialHandles.website.length > 5 && (
                  <a
                    href={
                      socialHandles.website.startsWith("http")
                        ? socialHandles.website
                        : `https://${socialHandles.website}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
          ...
          {showAgencyForm && (
            ...
            >
              ...
              >
                Are you represented by a modeling agency? If so, add their
                details below. This information will appear on your portfolio.
              </p>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                >
                  Agency Website{" "}
                  <span
                    style={{ color: "rgba(26, 26, 26, 0.4)", fontWeight: 400 }}
                  >
                    (optional)
                  </span>
                </label>
        ...
        <div style={styles.inputContainer}>
          ...
          <div style={styles.inputWrapper}>
            <textarea
              ...
              placeholder={
                activeTool === "location"
                  ? "Type your city and country (e.g., Los Angeles, USA)..."
                  : "Type a message or click a tool to get started..."
              }
