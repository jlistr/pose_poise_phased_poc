...
export function useOnboardingData({
  ...
  existingServices,
  existingPhotos,
}: UseOnboardingDataProps = {}): UseOnboardingDataReturn {
  ...
  const buildInitialServices = (): ServicesData => {
    ...
    const experienceLevel =
      firstService?.experience_level || DEFAULT_SERVICES_DATA.experienceLevel;
    const travelAvailable =
      firstService?.travel_available ?? DEFAULT_SERVICES_DATA.travelAvailable;
    const travelRadius =
      firstService?.travel_radius || DEFAULT_SERVICES_DATA.travelRadius;
    const tfpAvailable =
      firstService?.tfp_available ?? DEFAULT_SERVICES_DATA.tfpAvailable;
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && userId) {
      try {
        ...
        console.error("Failed to load onboarding state from local storage", err);
      }
      setIsLoaded(true);
    } else {
      setIsLoaded(true);
  ...
  useEffect(() => {
    if (isLoaded && typeof window !== "undefined" && userId) {
      localStorage.setItem(`onboarding_${userId}`, JSON.stringify(data));
    }
  ...
  const updatePhotoStatus = useCallback(
    (id: string, updates: Partial<PortfolioPhoto>) => {
      setData((prev) => ({
        ...prev,
  ...
  const addPhotos = useCallback(
    (files: FileList) => {
      ...
      if (currentCount >= MAX_PHOTOS) {
        alert("You have reached the maximum limit of 10 photos for the free tier.");
        return;
      }
      ...
      if (filesArray.length > availableSlots) {
        alert(`You can only add ${availableSlots} more photo(s). The remaining will be ignored.`);
      }

      const filesToAdd = filesArray.slice(0, availableSlots);

      const newPhotos: PortfolioPhoto[] = filesToAdd.map(
        (file, index) => ({
          id: generatePhotoId(),
  ...
  const updatePhotoCredit = useCallback(
    (id: string, field: "photographer" | "studio", value: string) => {
      setData((prev) => ({
        ...prev,
        photos: prev.photos.map((p) =>
  ...
  const retryPhotoUpload = useCallback(
    (id: string) => {
      ...
      if (photo?.file && photo.uploadStatus === "error") {
        uploadPhoto(photo);
      }
    },
  ...
  const lastSavedTemplateRef = useRef<string | null>(
    existingProfile?.selected_template || null,
  );
  ...
  const saveTemplate = useCallback(async () => {
    ...
    if (
      ...
      console.warn(
        "Invalid template selection, skipping save:",
        data.selectedTemplate,
      );
      return;
    }
  ...
  const saveProfile = useCallback(async () => {
    ...
    try {
      ...
      if (data.profile.location)
        formData.append("location", data.profile.location.trim());
      if (data.profile.instagram)
        formData.append("instagram", data.profile.instagram.trim());
      if (data.profile.tiktok)
        formData.append("tiktok", data.profile.tiktok.trim());
      if (data.profile.website)
        formData.append("website", data.profile.website.trim());
      if (data.profile.agency)
        formData.append("agency", data.profile.agency.trim());
  ...
  const saveServices = useCallback(async () => {
    ...
    try {
      const selectedCategories = data.services.categories.filter(
        (c) => c.selected,
      );
      const payload = {
        ...
        services: selectedCategories.map((cat) => ({
          ...
          pricing: {
            hourly: data.services.pricing.hourly
              ? Number(data.services.pricing.hourly)
              : null,
            half_day: data.services.pricing.halfDay
              ? Number(data.services.pricing.halfDay)
              : null,
            full_day: data.services.pricing.fullDay
              ? Number(data.services.pricing.fullDay)
              : null,
          },
        })),
