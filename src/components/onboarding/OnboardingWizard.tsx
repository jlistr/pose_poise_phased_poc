...
export function OnboardingWizard({
  ...
  const {
    ...
    isSavingProfile,
    isSavingServices,
  } = useOnboardingData({ userId, existingProfile, existingServices, existingPhotos });
  ...
  const handleLaunchPortfolio = useCallback(async () => {
    ...
    try {
      ...
      profileFormData.append(
        "username",
        data.profile.username.toLowerCase().trim(),
      );
      profileFormData.append("display_name", data.profile.displayName.trim());
      if (data.profile.location)
        profileFormData.append("location", data.profile.location.trim());
      if (data.profile.instagram)
        profileFormData.append("instagram", data.profile.instagram.trim());
      if (data.profile.tiktok)
        profileFormData.append("tiktok", data.profile.tiktok.trim());
      if (data.profile.website)
        profileFormData.append("website", data.profile.website.trim());
      if (data.profile.agency)
        profileFormData.append("agency", data.profile.agency.trim());
      ...
      if (stats.hairColor)
        profileFormData.append("hair_color", stats.hairColor);
      ...
      const templateToSave =
        data.selectedTemplate && VALID_TEMPLATES.includes(data.selectedTemplate)
      ...
      for (const photo of data.photos) {
          ...
          if (creditsChanged) {
            ...
            try {
              const updateResponse = await fetch(
                "/api/onboarding/update-photo-credits",
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
        ...
        if (
          photo.file &&
          (photo.uploadStatus === "pending" || photo.uploadStatus === "error")
        ) {
